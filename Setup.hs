...
import Distribution.Package ( PackageId, ComponentId, UnitId, packageVersion, packageName, newSimpleUnitId )
import Distribution.PackageDescription ( PackageDescription(), Executable(..) )
import Distribution.InstalledPackageInfo (sourcePackageId, installedComponentId)
...
generateBuildModule verbosity pkg lbi = do
  withLibLBI pkg lbi $ \_ libcfg -> do
    withExeLBI pkg lbi $ \exe clbi -> do
      ...
      rewriteFileEx verbosity (dir </> "Build_" ++ exeName' exe ++ ".hs") $ unlines
  ...
  where
    ...
    transDeps xs ys =
      ...
      where
        ...
        allInstPkgIds = map (newSimpleUnitId . installedComponentId) $ allPackages allInstPkgsIdx
